...
namespace PTPInputStreamingOutputGenerator
{
    class Program
    {
        static readonly string MASTER_IP = "10.100.35.79"; // IP address (or hostname) of the 3050 LAN-XI module (PTP master)
        static readonly string SLAVE_IP = "10.100.35.192"; // IP address (or hostname) of the 3160 LAN-XI module (PTP slave)
        ...
        static void Main(string[] args)
        {
            ...
            string syncParametersMaster = File.ReadAllText(@"PTP_3050_3160_SyncParametersMaster.json");
            master.PutRequestWithPath("/rest/rec/syncmode", syncParametersMaster);
            string syncParametersSlave = File.ReadAllText(@"PTP_3050_3160_SyncParametersSlave.json");
            ...
            string openParameters = File.ReadAllText(@"PTP_3050_3160_OpenParameters.json");
            slave.PutRequestWithPath("/rest/rec/open", openParameters);
            master.PutRequestWithPath("/rest/rec/open", openParameters);

            // Prepare generator
            string outputChannelStart = File.ReadAllText(@"PTP_3050_3160_OutputChannelStart.json");
            slave.PutRequestWithPath("/rest/rec/generator/prepare", outputChannelStart);

            // Configure generator channels
            string outputChannelConfiguration = File.ReadAllText(@"PTP_3050_3160_OutputChannelSetup.json");
            ...
            string MasterInputChannelConfiguration = File.ReadAllText(@"PTP_3050_3160_MasterInputChannelSetup.json");
            string SlaveInputChannelConfiguration = File.ReadAllText(@"PTP_3050_3160_SlaveInputChannelSetup.json");
