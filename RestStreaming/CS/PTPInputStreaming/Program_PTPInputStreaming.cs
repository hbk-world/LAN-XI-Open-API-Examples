...
namespace PTPInputStreaming
{
    class Program
    {
        static readonly string MASTER_IP = "10.100.35.79"; // IP address (or hostname) of the first LAN-XI module (PTP master)
        static readonly string SLAVE_IP = "10.100.35.192"; // IP address (or hostname) of the second LAN-XI module (PTP slave)
        ...
        static int[,] samplesReceived = {{0, 0, 0, 0},{0, 0, 0, 0}}; // Used to count the number of samples received - for demo purpose. Array of one array per LAN-XI module, containing one integer per input channel.
        static Socket[] sock; // Array of sockets used when fetching samples from the LAN-XI modules

        static ArrayList[,] outputSamples = {{new ArrayList(), new ArrayList(), new ArrayList(), new ArrayList()},  // Buffers for storing samples fetched from the LAN-XI modules. Array of one array per LAN-XI module, containing one ArrayList per input channel.
        ...
        static void Main(string[] args)
        {
            ...
            // Set synchronization mode on the LAN-XI modules. The body tells which module is master and which is slave. Body for each module is prepared in PTPInputStreaming_SyncParametersMaster.json and PTPInputStreaming_SyncParametersSlave.json
            // For this command, the MASTER should be treated first, then the slave(s)
            string syncParametersMaster = File.ReadAllText(@"PTPInputStreaming_SyncParametersMaster.json");
            master.PutRequestWithPath("/rest/rec/syncmode", syncParametersMaster);
            string syncParametersSlave = File.ReadAllText(@"PTPInputStreaming_SyncParametersSlave.json");
            ...
            // Open the Recorder application on the modules. The same body is sent to both modules, and is prepared in PTPInputStreaming_OpenParameters.json
            string openParameters = File.ReadAllText(@"PTPInputStreaming_OpenParameters.json");
            ...
            // The body has been prepared and stored in PTPInputStreaming_InputChannelSetup.json. In this example the setup is identical for the two modules, but it may differ as needed.
            string inputChannelConfiguration = File.ReadAllText(@"PTPInputStreaming_InputChannelSetup.json");
